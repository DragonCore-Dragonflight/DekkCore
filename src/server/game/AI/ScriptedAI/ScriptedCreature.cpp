    ...
    for (Player* player : playersNearby)
    {
        ...
        if (pangle >= 0 && pangle <= float(M_PI_4))
        {
            if (BotGroupAI* pGroupAI = dynamic_cast<BotGroupAI*>(player->GetAI()))
            {
                ...
                Position fleePos = Position(center->GetPositionX() + fleeRange * cosf(fleeAngle),
                    center->GetPositionY() + fleeRange * sinf(fleeAngle), player->GetPositionZ(), player->GetOrientation());
