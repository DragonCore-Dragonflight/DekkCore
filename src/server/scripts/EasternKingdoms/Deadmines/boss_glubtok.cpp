        ...
        struct boss_glubtokAI : public BossAI
        {
            ...
            void UpdateAI(uint32 diff) override
            {
                ...
                if (me->GetHealthPct() <= 50 && stage == 0)
                {
                    ...
                    Talk(SAY_HEAD1);
                    events.RescheduleEvent(EVENT_ARCANE_POWER1, 2s);
                    return;
                }
                ...
                if (uint32 eventId = events.ExecuteEvent())
                {
                    ...
                    case EVENT_ARCANE_POWER2:
                        ...
                        me->AttackStop();
                        me->RemoveAllAuras();
                        events.RescheduleEvent(EVENT_ARCANE_POWER3, 1s);
                        break;
                    case EVENT_ARCANE_POWER3:
                        SetCombatMovement(false);
                        DoCast(me, SPELL_ARCANE_POWER, true);
                        Talk(SAY_ARCANE_POWER);
                        events.RescheduleEvent(EVENT_BLOSSOM, 5s);
                        break;
                    case EVENT_BLOSSOM:
                    ...
                    case EVENT_FIST_OF_FLAME:
                        ...
                        Talk(SAY_FIRE);
                        events.RescheduleEvent(EVENT_BLINK, 20s);
                        events.RescheduleEvent(EVENT_FIST_OF_FROST, 21s);
                        break;
                    case EVENT_FIST_OF_FROST:
                        DoCast(me, SPELL_FIST_OF_FROST);
                        Talk(SAY_FROST);
                        events.RescheduleEvent(EVENT_BLINK, 20s);
                        events.RescheduleEvent(EVENT_FIST_OF_FLAME, 21s);
                        break;
                    case EVENT_BLINK:
                        if (auto target = SelectTarget(SelectTargetMethod::Random, 0, 100.0f, true))
                        {
