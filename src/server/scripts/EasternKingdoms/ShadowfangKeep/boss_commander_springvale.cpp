...
class boss_commander_springvale : public CreatureScript
{
    public:
        ...
        struct boss_commander_springvaleAI : public BossAI
        {
            ...
            {
                if (IsHeroic())
                    events.RescheduleEvent(EVENT_ADDS, 45s);
                events.RescheduleEvent(EVENT_MALEFIC_STRIKE, 9s);
                events.RescheduleEvent(EVENT_DESECRATION, 15s);
            ...
            void UpdateAI(uint32 uiDiff) override
            {
                ...
                while(uint32 eventId = events.ExecuteEvent())
                {
                    ...
                    case EVENT_DESECRATION:
                        ...
                        events.RescheduleEvent(EVENT_DESECRATION, 25s);
                        break;
                    ...
                    case EVENT_ADDS:
                        Talk(SAY_ADDS);
                        me->SummonCreature(NPC_SPRINGVALE_OFFICER, addSpawnPos[0],TEMPSUMMON_CORPSE_DESPAWN, 10s);
                        me->SummonCreature(NPC_SPRINGVALE_GUARD, addSpawnPos[1],TEMPSUMMON_CORPSE_DESPAWN, 10s);
                        events.RescheduleEvent(EVENT_ADDS, 45s);
                        break;
                ...
                if (Aura* unholypower = me->GetAura(DUNGEON_MODE(SPELL_UNHOLY_POWER, SPELL_UNHOLY_POWER_H)))
                {
                    if (unholypower->GetStackAmount() >= 3)
                    {
                        ...
                        if (IsHeroic())
                        {
                            ...
                            {
                                case 0:
                                    events.RescheduleEvent(EVENT_WORD_OF_SHAME, 1s);
                                    break;
                                case 1:
                                    events.RescheduleEvent(EVENT_SHIELD_OF_PERFIDIOUS, 1s);
                                    break;
                            }
                        }
                        else
...
class npc_springvale_wailing_guardsman : public CreatureScript
{
    public:
        ...
        struct npc_springvale_wailing_guardsmanAI : public ScriptedAI
        {
            ...
            void JustEngagedWith(Unit* pWho) override
            {
                events.RescheduleEvent(EVENT_MORTAL_STRIKE, 3s);
                events.RescheduleEvent(EVENT_SCREAMS_OF_THE_PAST, 15s);
                events.RescheduleEvent(EVENT_UNHOLY_EMPOWERMENT, 15s);
            }
...
class npc_springvale_tormented_officer : public CreatureScript
{
    public:
        ...
        struct npc_springvale_tormented_officerAI : public ScriptedAI
        {
            ...
            {
                events.RescheduleEvent(EVENT_UNHOLY_EMPOWERMENT, 15s);
                events.RescheduleEvent(EVENT_FORSAKEN_ABILITY, 20s);
                events.RescheduleEvent(EVENT_SHIELD_WALL, 25s);
            }
...
class npc_springvale_desecration_bunny : public CreatureScript
{
    public:
        ...
        {
            npc_springvale_desecration_bunnyAI(Creature* pCreature) : ScriptedAI(pCreature)
            {
            }
