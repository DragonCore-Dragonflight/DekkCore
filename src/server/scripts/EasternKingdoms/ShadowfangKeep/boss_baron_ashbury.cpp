...
class boss_baron_ashbury : public CreatureScript
{
    public:
        ...
        struct boss_baron_ashburyAI : public BossAI
        {
            ...
            {
                Talk(SAY_AGGRO);
                events.RescheduleEvent(EVENT_PAIN_AND_SUFFERING, 8s);
                events.RescheduleEvent(EVENT_ASPHYXIATE, 30s);
                if (IsHeroic())
                    events.RescheduleEvent(EVENT_WRACKING_PAIN, 15s);
            ...
            void UpdateAI(uint32 uiDiff) override
            {
                ...
                if (IsHeroic())
                {
                    if (!HealthAbovePct(20) && !bArchangel)
                    {
                        ...
                        events.RescheduleEvent(EVENT_CALAMITY, 3s);
                    }
                }
                ...
                while(uint32 eventId = events.ExecuteEvent())
                {
                    switch(eventId)
                    {
                        ...
                        case EVENT_ASPHYXIATE:
                            ...
                            events.RescheduleEvent(EVENT_STAY_OF_EXECUTION, 6s);
                            events.RescheduleEvent(EVENT_ASPHYXIATE, 45s);
                            break;
                        case EVENT_STAY_OF_EXECUTION:
                            Talk(SAY_STAY_OF_EXECUTION);
                            DoCast(SelectTarget(SelectTargetMethod::MinDistance), SPELL_STAY_OF_EXECUTION);
                            bCombo = false;
                            break;
                        case EVENT_CALAMITY:
                        ...
                        case EVENT_WRACKING_PAIN:
                            ...
                            events.RescheduleEvent(EVENT_WRACKING_PAIN, 30s);
                            break;
                    }
                }
