...
public:
    ...
    struct boss_houndmaster_braunAI : public BossAI
    {
        ...
        {
            Talk(SAY_AGGRO_1);
            events.RescheduleEvent(EVENT_AGGRO, 5s);
            events.RescheduleEvent(EVENT_PIERCING_THROW, 12s);
            events.RescheduleEvent(EVENT_DEATH_BLOSSOM, 18s);
        ...
        void DamageTaken(Unit* attacker, uint32& damage, DamageEffectType /*damageType*/, SpellInfo const* /*spellInfo = nullptr*/) override
        {
            if (damage >= me->GetHealth())
            {
                ...
                {
                    DeathEvent = true;
                    events.RescheduleEvent(EVENT_FINISH, 1s);
                }
            }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (uint32 eventId = events.ExecuteEvent())
            {
                ...
                case EVENT_PIERCING_THROW:
                    DoCast((SelectTarget(SelectTargetMethod::Random, 0, 50, true)), SPELL_PIERCING_THROW);
                    events.RescheduleEvent(EVENT_PIERCING_THROW, 12s);
                    break;
                case EVENT_DEATH_BLOSSOM:
                    DoCast((SelectTarget(SelectTargetMethod::Random, 0, 50, true)), SPELL_DEATH_BLOSSOM);
                    events.RescheduleEvent(EVENT_DEATH_BLOSSOM, 18s);
                    break;
                case EVENT_FINISH:
                    ...
                    events.RescheduleEvent(EVENT_DIE, 5s);
                    break;
                }
            }
...
public:
    ...
    struct npc_obedient_houndAI : public ScriptedAI
    {
        ...
        {
            if (action == 1)
                events.RescheduleEvent(EVENT_DOGS_RAGE, 1s);
            ...
            if (action == 2)
            {
               ...
               // DoZoneInCombat(me, 60.0f);
                events.RescheduleEvent(EVENT_DOGS_ATTACK, 1s);
            }
        }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (uint32 eventId = events.ExecuteEvent())
            {
                ...
                case EVENT_DOGS_ATTACK:
                    DoCast((SelectTarget(SelectTargetMethod::Random, 0, 50, true)), SPELL_HUNGRY_DOG, true);
                    break;
