...
class spell_dalaran_teleportation : public SpellScript
{
    ...
    void EffectTeleportDalaranKarazhan(SpellEffIndex effIndex)
    {
        ...
        {
            if (player->GetLevel() < 60 || player->GetQuestStatus(QUEST_BLINK_OF_AN_EYE) != QUEST_STATUS_INCOMPLETE)
...
public:
    ...
    void OnTempSummonNPC(uint32 insideNpc, Player* player)
    {
        ...
        if (auto creature = player->FindNearestCreature(_insideNpc, 100.0f))
...
//    }
    ...
    void OnPlayerEnterZone(Player* player) override
    {
        ...
        {
            if (auto quest = sObjectMgr->GetQuestTemplate(QUEST_A_THE_HAND_OF_FATE))
        ...
        {
            if (auto quest = sObjectMgr->GetQuestTemplate(QUEST_H_THE_HAND_OF_FATE))
...
{
    PrepareSpellScript(spell_margoss_rock_throw);
